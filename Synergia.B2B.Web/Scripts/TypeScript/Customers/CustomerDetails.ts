...
class CustomerDetails {
    ...
    public reloadAllDataForLoggedUser() {
    ...
    public loadCustomerDetailsForLoggedUser() {
        $.ajax({
            url: '/Api/ApiCustomers/GetCustomerDetailsForLoggedUser',
            data: { customerId: this.customerId },  //id to parametr metody GetInstallationObject z Web servisu API
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            type: 'GET',
            success: (result) => {
    ...
    public loadCustomerDetailsByCustomerId() {
        $.ajax({
    ...
    public loadUserListForCustomer() {
        if (!this.list) {
