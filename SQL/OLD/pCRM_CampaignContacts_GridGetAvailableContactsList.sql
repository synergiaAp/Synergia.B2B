...
/****** Object:  StoredProcedure [HM].[pCRM_CampaignContacts_GridGetAvailableContactsList]    Script Date: 13.05.2018 19:09:11 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [HM].[pCRM_CampaignContacts_GridGetAvailableContactsList] 
 @CampaignId INT,
...
BEGIN
 ...
 LEFT JOIN HM.CRM_CampaignContacts CC ON CC.CampaignId = @CampaignId AND CC.ContactId = C.Id
	AND CC.IsDeleted = 0
 WHERE (ISNULL(@Search, '') = '' 
  ...
  AND ISNULL(C.Email, '') != '' 
  AND CC.Id IS NULL
  AND (ISNULL(@ContactType, '') = '' OR C.CustomerType = @ContactType)
